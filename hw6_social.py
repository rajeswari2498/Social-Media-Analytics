...
def mostCommonHashtags(hashtags, count):
    ...
    empty1_dict={}
    counts =0
    empty_dict=sorted(hashtags, key= hashtags.get, reverse=True)
    for each in empty_dict:
        # print(each)
        if counts<count: 
            empty1_dict[each]=hashtags[each]
            counts=counts+1
    return empty1_dict
...
def getHashtagSentiment(data, hashtag):
    ...
    average_tagsentiment=sum(sentiment_list)/count
    # print(average_tagsentiment)
    return average_tagsentiment
